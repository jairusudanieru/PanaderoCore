...
public class Reply implements TabCompleter, CommandExecutor {
   ...
   public boolean onCommand (@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, String[] args) {
      ...
      if (!ChatClass.lastReceived.containsKey(player.getUniqueId()) || ChatClass.lastReceived.get(player.getUniqueId()) == null) {
         sender.sendMessage("You have no one to reply to!");
         return true;
      }

      Player target = Bukkit.getPlayer(ChatClass.lastReceived.get(player.getUniqueId()));
      if (target == null) {
